...
namespace DigitalHijinks.MiKeys.Managers
{
    public  class EntryManager
    {
        ...
        public void Load(Texture2D CM)
        {
            keyTex = ModContent.Request<Texture2D>("VirtualJoystick/KeyExample").Value;
        }
        ...
        public void Update(GameTime gt)
        {
            ...
            if (GetKeys(out char[] key))
            {
                ...
                // 检查 CapsLock 和 NumLock 状态
                CapsLockOn = (keyboardState.IsKeyDown(Keys.CapsLock)) || (keyboardState.IsKeyUp(Keys.CapsLock) && !keyboardState.IsKeyDown(Keys.CapsLock));
                NumLockOn = (keyboardState.IsKeyDown(Keys.NumLock)) || (keyboardState.IsKeyUp(Keys.NumLock) && !keyboardState.IsKeyDown(Keys.NumLock));
            ...
            foreach (UIKey u in UIKeys)
            {
                MouseState ms1 = Mouse.GetState(); // 获取当前鼠标状态

                if (u.Bounds.Contains(ms.X, ms.Y)) // 使用 X 和 Y 属性
        ...
        public void DrawKeyboard(SpriteBatch spriteBatch)
        {

            if (VirtualGamepad.text)
            {
                SF = ModContent.Request<SpriteFont>("VirtualJoystick/Fonts/FSEX302").Value;

                // 计算输入框的原点，宽度的一半和高度的一半
                Vector2 inputOrigin = new(InputSize.Width / 2, (int)(16 * _scale / 2));

                // 绘制输入框的矩形区域，白色背景和黑色边框
                Globals.DrawRect(SBX,
                    ScreenCenter - inputOrigin - new Vector2(6 * _scale, 65 * _scale),
                    ScreenCenter + inputOrigin + new Vector2(0, -(61 * _scale)),
                    Color.White, Color.Black, _scale, true);

                // 在输入框内绘制输出字符串
                SBX.DrawString(SF,
                    _outputString,
                    ScreenCenter - inputOrigin - new Vector2(2 * _scale, (16 * 4) * _scale),
                    Color.White, 0, Vector2.Zero, _scale, SpriteEffects.None, 0);

                // 如果光标应该显示且输出字符串长度小于最大长度，则绘制光标
                if (_caretShow && _outputString.Length < _maxLength)
                    SBX.DrawString(SF,
                        "|",
                        ScreenCenter - inputOrigin - new Vector2((2 * _scale), (16 * 4) * _scale) +
                        new Vector2(2 * _scale + (_outputString.Length * 8 * _scale), 0),
                        Color.WhiteSmoke, 0, Vector2.Zero, _scale, SpriteEffects.None, 0);
            }
        ...
        {
            // 如果当前对象不活跃，则直接返回，不执行后续代码
            if (!IsActive) return;

            // 结束当前的绘制批处理
            spriteBatch.End();
            // 开始一个新的绘制批处理，设置排序模式、混合状态和采样状态
